...
```
Boxplot is n't good diagram to visualize regression relationship. It works for ANOVA though 
...
ggplot(reg_dfw, 
  ...
  geom_ribbon(data = nd, aes(x = sr_ksha, 
                             ymin = mod3_quad_lcl...9,
                             ymax = mod3_quad_upl...10),
              fill = "gray", 
              alpha = 0.5, 
              inherit.aes = FALSE) 
...
## a) Model
nmle function to run non-linear model
xs = break point in x-axis where after certain point you don't see any return with addition of seed rate
muti-non-linear model = weak for prediction 
...
Muti-model interefernece can be done** if you need look for it. 
